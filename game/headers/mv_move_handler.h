...
#define LOOP_HEIGHT(screen_height, height, x,y) for(int i = 0; i < screen_height -1 && i+y < height;  i++)
#define LOOP_WIDTH(screen_width,width, x,y) for(int j = DEFAULT_MAX_INFOBAR_WIDTH; j < screen_width -1 && j+x < width;  j++)
#define REDRAW_MAP(gs,c,current_zone,draw_screen, x,y)  LOOP_HEIGHT(gs->num_cols,current_zone->height, x,y)  LOOP_WIDTH(gs->num_rows,current_zone->width, x,y) mvwprintw(draw_screen,i,j,(current_zone->tiles[(c->position.global_y - (c->position.local_y))+i][(c->position.global_x - c->position.local_x )+j].content));  mvwprintw(draw_screen,c->position.local_y, c->position.local_x, c->representation); //c->standing_on[0] = current_zone->tiles[c->position.global_y][c->position.global_x].content[0]; current_zone->tiles[c->position.global_y][c->position.global_x].content[0] = c->representation[0];
