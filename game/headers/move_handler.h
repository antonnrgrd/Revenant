...
#define REDRAW_MAP(c,current_zone,draw_screen, x,y, max_x, max_y)  LOOP_HEIGHT(current_zone->height, x,y, max_x, max_y)  LOOP_WIDTH(current_zone->width, x,y, max_x, max_y) mvprintw(draw_screen,i,j,(current_zone->tiles[(c->position.global_y - (c->position.local_y))+i][(c->position.global_x - (c->position.local_x))+j].content));  mvprintw(draw_screen,c->position.local_y, c->position.local_x, c->representation); c->standing_on[0] = current_zone->tiles[c->position.global_y][c->position.global_x].content[0]; current_zone->tiles[c->position.global_y][c->position.global_x].content[0] = c->representation[0];
