...
mvwprintw(game_state->logs[MAIN_SCREEN],0,0, "%s%s%s%d%s", "Pickup ", i_derive_item_name[game_state->current_zone->tiles[global_y][global_x].entry->item_holder->item->kind], " amount: ", game_state->current_zone->tiles[global_y][global_x].entry->item_holder->amount, " ? [y/n/a/d]");  
  */
  ...
  switch(response){
   case 'y':
     result = inv_add_item(game_state->current_zone->tiles[global_y][global_x].entry->item_holder, (struct U_Hashtable *)c->additional_info, c,NO);
...
int move_response_initiate_dialoge(int global_x, int global_y,int local_x, int local_y, C_Creature *c,Game_State *game_state){
  game_state->dialogue_manager = (Dia_Dialogue_Manager *)game_state->current_zone->tiles[global_y][global_x].foe;
  dia_loop_dialogue((Dia_Dialogue_Manager *)game_state->current_zone->tiles[global_y][global_x].foe, game_state);
  game_state->dialogue_manager = NULL;
  return CONTINUE_TURN;
}
