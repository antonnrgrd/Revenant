...
\subsection{Policy on when to include header files}
...
\end{lstlisting}
Then it is to be included only \textit{exactly} the momenet when it become relevant, that is to say, when the code first refers to functionality/a declaration, made in the header it is including.  \\

This is because 
\begin{enumerate}
\item It makes it transparent why the include statement was made.
\item Furthermore, avoids cluttering with unecessary include statements, potentially causing compiling errors and makes it more easy in reasoning what has been defined in which parts of the code.
\end{enumerate}
...
\subsubsection*{Handling of picking dialogue options flow and the subsequent design of the database}
There are to be defined a total of at least 3 tables 

\begin{itemize}
\item dia\_dialogue\_interactions - intended to correspond almost directly to a Dia\_Dialogue\_Manager manager, minus meta data about the current state of the dialogue session such as weather the player has reached the end of the current dialogue file, weather the file fits into a single window etc.

\item dia\_dialogue\_selected\_option\_handler - inteded to manage the overall flow of updating the dialogue. When a player selects an option, it will be used to decide what the next dialogue id should be (if any), what checks are to be made, if any and the next dialogue id's provided the player fails or suceeds the checks respectively.

\item dia\_dialogue\_option\_triggers defines the set of triggers to fire 
\end{itemize}

The general flow of the dialogue flow is to be as follows

\begin{enumerate}
\item Assuming the player picked a valid dialogue option, query dia\_dialogue\_selected\_option\_handler. In this table, it will check what kind of check to do, if any, to decide what the next dialogue file should be. If the check is to simply continue dialogue, just read in what the next dialogue flag is set to be which will be stored in the success check flag value. If the check is to exit the dialogue, store the exit flag in the current dialogue id, then when the game loop sees, that the current dialogue id is actually an exit flag, it will quit the dialogue session. Provided a check is defined, the success or failure is decided by another query in the current callback for the query to dia\_dialogue\_selected\_option\_handler. The result of the check is in the relevant callback stored in the query manager struct. 

\item In conunction
\end{enumerate}

\subsubsection*{Design choices for the dialogue flow}
This dialogue / database design opens op for a hefty bit of redundancy in the dia\_dialogue\_selected\_option\_handler table - in that it stores both the id for the next dialogue file if a check is succesfull and if the check fails, but the failure id will be redundant if there is no check (the dialogue trivially progresses to the next dialogue id) and both will be redundant if there is no check and no next dialogue id (selecting said option culminates in exiting dialogue) But the alternative would be to split it into yet more tables - requireing more indexes, more queries, more logic and more overall management. Hence, we opted to introduce a fair amount of redundancy to skimp design / coding logic. \\

It has by design been decided that when it comes to dynamically deciding what the next dialogue id should be read in, there can only be at most \underline{one} binary check. So it's not possible for there to be multiple checks to decide what the next dialoge id should be and it is to be decided by a successfull check or a failure. The motivation for this is choice is that otherwise, the logical complexity for maintaining all the different checks would be mind boggling to follow and troubleshoot, given they're stored and retried in SQL queries.¨
...
In addition to the naming standards specified in section \ref{sec:naming}, the naming standards for the database components adheres to the following set of rules and conventions. \newline
...
All structs, that represent or or more value(s) derived from a query, shall be named *\_Qresult, to indicate it is the result of a query, analogous to the naming convention for the macros. \\

If there is a 1-to-1 correspondence between the value(s) returned from a query and a desired struct in said context, the dbr\_reader is allowed to return said struct directly, rather than a *\_Qresult struct.

\paragraph*{Naming standard for actual database objects}
The naming standard for the database objects like tables, triggers, etc. Follow the rules defined in section \ref{sec:naming}.
...
A1 & B1 & C1 \\ \hline
\multicolumn{2}{|c|}{Merged Columns} & C3 \\ \hline
D1 & D2 & D3 \\ \hline
\multicolumn{3}{|c|}{All Columns Merged} \\ \hline
F1 & G1 & H1 \\ \hline
\end{tabular}
In this subsection, we will list all the 
\begin{table}[h]
\begin{tabular}{ |p{4cm}|p{1.2cm}|p{3cm}|p{3cm}|  }
\hline
 \multicolumn{4}{|c|}{\textbf{Table:dia\_dialogue\_interactions}} \\
 \hline
  \multicolumn{3}{|c|}{Purpose} & {Retrieves the number of dialogue options for the current dialogue id.} \\
  \hline
 dialogue\_folder\_id & 0 & int & The id of the dialogue folder \\
 current\_dialogue\_id & 1 & int & A dialogue file in the dialogue folder in question. Identified by an id\\
 num\_dialogue\_options & 2 & int & The number of dialogue options available for the dialogue\_folder\_id + current\_dialogue\_id \\
 \hline
  \multicolumn{4}{|c|}{\textbf{Indexes, keys and constraints}} \\
   \hline
   \multicolumn{4}{|p{5cm}|}{PRIMARY KEY(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id)} \\
      \multicolumn{4}{|p{5cm}|}{CREATE INDEX IF NOT EXISTS(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id)} \\
 \hline
\multicolumn{4}{|c|}{\textbf{Dependencies of dia\_dialogue\_interactions}}  \\
\hline
\multicolumn{4}{|c|}{Table dependencies of dia\_dialogue\_interactions}  \\
\hline
\multicolumn{3}{|c|}{Table name} & Role \\
\hline
\multicolumn{3}{|c|}{dia\_dialogue\_selected\_option\_consequence\_handler - \ref{Tab:diadialogueselectedoptionconsequencehandler}} & If any checks are to be made as a consequence of a selected dialogue options, e.g skill check, trigger check this table will also have to be read from. \\
\hline
\end{tabular}
\end{table}

\begin{table}[h]
\begin{tabular}{ |p{6cm}|p{1.2cm}|p{3cm}|p{3.6cm}|  }
\hline
 \multicolumn{4}{|c|}{\textbf{Table:dia\_dialogue\_selected\_option\_handler}} \\
 \hline
  \multicolumn{3}{|c|}{Purpose} & {Retrieves the next dialogue id screen, based on whether the check was successful or not.} \\
 \hline
 Column name & Column index & Data type & Description \\
  \hline
 dialogue\_folder\_id & 0 & int & dialogue folder id of the Dia\_Dialogue\_Manager used for the current dialogue session.\\
 current\_dialogue\_id & 1 & int & The current dialogue\_id file being read from the dialogue folder.\\
 selected\_option & 2 & int & The selected dialogue option for the current dialogue\_folder\_id and current\_dialogue\_id context \\
check\_type & 3 & int & What type of check to commence \\
 next\_dia\_id\_if\_check\_success & 4 & int & The next current dialogue to read in, provided the passed the checks, if any. \\
 next\_dia\_id\_if\_check\_fail & 5 & int & The next current dialogue to read in, provided the passed the checks, if any. \\
\hline 
\multicolumn{4}{|c|}{\textbf{Indexes, keys and constraints}}  \\
\hline
   \multicolumn{4}{|p{5cm}|}{PRIMARY KEY(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id, selected\_option} \\
      \multicolumn{4}{|p{5cm}|}{CREATE INDEX IF NOT EXISTS(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id, selected{\_option}} \\ 
   \hline
   \multicolumn{4}{|p{5cm}|}{FOREIGN KEY(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id), REFERENCES dia\_dialogue\_interactions(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id)} \\      
 \hline
\multicolumn{3}{|c|}{Table name} & Role \\
\hline
\multicolumn{3}{|c|}{selected\_dialogue\_option} & If any checks are to be made as a consequence of a selected dialogue options, e.g skill check, trigger check this table will also have to be read from. \\
\hline
\end{tabular}
\end{table}

\begin{table}[h]
\begin{tabular}{ |p{6cm}|p{1.2cm}|p{3cm}|p{3.6cm}|p{3.6cm}|  }
\hline
 \multicolumn{4}{|c|}{\textbf{Table:dia\_dialogue\_option\_triggers}} \\
  \hline
 \multicolumn{3}{|c|}{Purpose} & {Applies all the triggers applicable to a dialogue option, based on whether the check was successful or not} \\
 \hline
 Column name & Column index & Data type & Description \\
  \hline
 dialogue\_folder\_id & 0 & int & dialogue folder id of the Dia\_Dialogue\_Manager used for the current dialogue session.\\
 current\_dialogue\_id & 1 & int & The current dialogue\_id file being read from the dialogue folder.\\
 selected\_option & 2 & int & The selected dialogue option for the current dialogue\_folder\_id and current\_dialogue\_id context \\
 on\_outcome & 3 & int & Is the trigger applied on a success or failure. \\
 trigger\_type & 4 & int & what type of trigger to be applied, based on the selected choice of the player \\
 \hline
\multicolumn{4}{|c|}{\textbf{Indexes, keys and constraints}}  \\
\hline
   \multicolumn{4}{|p{5cm}|}{PRIMARY KEY(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id, selected\_option, on\_outcome, trigger\_type),} \\
      \multicolumn{4}{|p{5cm}|}{CREATE INDEX IF NOT EXISTS(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id, selected{\_option},on{\_outcome}} \\ 
   \hline
   \multicolumn{4}{|p{5cm}|}{FOREIGN KEY(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id), REFERENCES dia\_dialogue\_interactions(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id)} \\      
\multicolumn{3}{|c|}{Table name} & Role \\
\hline
...
\begin{tabular}{ |p{6cm}|p{1.2cm}|p{3cm}|p{3.6cm}|p{3.6cm}|  }
\hline
 \multicolumn{4}{|c|}{\textbf{Table:dia\_trigger\_set\_initial\_dialogue}} \\
 \hline
 \multicolumn{3}{|c|}{Purpose} & {A trigger table that sets a new  inital dialogue id for the given dialogue manager} \\
 \hline
 Column name & Column index & Data type & Description \\
  \hline
 dialogue\_folder\_id & 0 & int & dialogue folder id of the Dia\_Dialogue\_Manager used for the current dialogue session.\\
 current\_dialogue\_id & 1 & int & The current dialogue\_id file being read from the dialogue folder.\\
 selected\_option & 2 & int & The selected dialogue option for the current dialogue\_folder\_id and current\_dialogue\_id context \\
 on\_outcome & 3 & int & Is the initial id set based on the success or failure of the check. \\
 target\_dialogue\_folder\_id & 4 & int & Which dialogue folder is having its initial id set \\
 new\_initial\_id & 5 & int & The new initial id for the dialogue folder \\
 \hline
\multicolumn{4}{|c|}{\textbf{Indexes, keys and constraints}}  \\
\hline
   \multicolumn{4}{|p{5cm}|}{PRIMARY KEY(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id, selected\_option, on\_outcome, target\_dialogue\_folder\_id),} \\
      \multicolumn{4}{|p{5cm}|}{CREATE INDEX IF NOT EXISTS(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id, selected{\_option}, on{\_outcome}} \\ 
   \hline
   \multicolumn{4}{|p{5cm}|}{FOREIGN KEY(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id), REFERENCES dia\_dialogue\_interactions(dialogue{\_}folder{\_}id, current{\_}dialogue{\_}id)} \\      
\multicolumn{3}{|c|}{Table name} & Role \\
